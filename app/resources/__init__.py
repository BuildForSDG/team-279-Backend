...
def create_or_update_resource(**kwargs):
    ...
    Arguments:
        ...
        kwargs["resource_type"]: The type of resource, e.g tender, company.
    """
    try:
        ...
        if kwargs["create"]:
            if kwargs["resource_type"] == "user":
                ...
                response = {"message": "You have successfully signed up", "token": auth_token.decode()}
            else:
                response = marshal(kwargs["resource"], kwargs["serializer"])
                message = {"message": "You have successfully created a new " + kwargs["resource_type"]}
                response.update(message)
            return response, 201
        else:
            response = marshal(kwargs["resource"], kwargs["serializer"])
            message = {"message": "You have successfully edited the " + kwargs["resource_type"]}
            response.update(message)
            return response
...
def delete_resource(resource, **kwargs):
    ...
    db.session.commit()
    return {"message": "You have successfully deleted the " + kwargs["resource_type"] + " with the following ID: " + kwargs["id"]}
